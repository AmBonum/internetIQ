...
3. **Follow the same conventions the generator uses** (`.claude/agents/playwright-test-generator.md` § 3–§ 7). Banned patterns are still banned during a fix:
   ...
   - **`test.step(...)` wrapping is preserved.** Every action and verification in a spec is wrapped in `test.step("English imperative title", async () => { … })` — see the generator agent's § 2 step 5. When healing, keep the step structure. If you split a step in two during a fix, give each new step its own title that matches the plan's clauses. Bare `await page.click(...)` or `await expect(...)` calls outside a `test.step(...)` block are forbidden.
