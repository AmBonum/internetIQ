...
CREATE TABLE IF NOT EXISTS public.test_sets (
  ...
  -- BEFORE INSERT/UPDATE trigger below — Postgres CHECK constraints
  -- cannot contain subqueries (would error 0A000).
);
...
CREATE TRIGGER test_sets_question_id_len_trg
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.check_test_sets_question_id_len();
